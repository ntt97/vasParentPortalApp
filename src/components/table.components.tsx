...
const Table = (props: any) => {
  const _renderCell = (cellData: any, col: any, index: number) => {
    ...
    return (
      <View
        key={dataIndex}
        style={[styles.cell, style, index % 2 == 0 ? cellStart : cellEnd]}>
  ...
  const _renderRow = (rowData: any, index: number) => {
    ...
    return (
      <View
        ...
        style={[
          styles.row,
          index % 2 ? {backgroundColor: colors.whiteSmoke} : {},
        ]}>
        {columns.map((col: any, columnIndex: number) =>
          _renderCell(rowData[col.dataIndex], col, columnIndex),
        )}
      </View>
    );
  };
