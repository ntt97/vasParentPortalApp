...
import { getCurrentNotify, setIconNotification, setLoadingNotify } from 'actions/notification.action';
...
const DEFAULT_PROPS = {
    ...
    onLinkPress: (evt: any, href: any) => { Linking.openURL(href); },
    debug: false,
...
const NotificationDetail = (props: any) => {
  ...
  const current: any = useSelector<RootState>((state: RootState) => state.notification.current);
  const eventNotSeen: any = useSelector<RootState>((state: RootState) => state.notification.eventNotSeen);
  ...
  if (current && current.eventDetails && Array.isArray(current.eventDetails)) {
    eventDetail = current.eventDetails.find((event: any) => event.languageId === props.currentLanguage);
  }
      ...
      <ScrollView style={{ flex: 1}}>
        {eventDetail && !!eventDetail.description ? (
          <ViewVertical horizontal style={styles.itemDescription}>
            <HTML tagsStyles={{span:{lineHeight:25}}} html={eventDetail.description} {...DEFAULT_PROPS} />
          </ViewVertical>
        ) : null}
      </ScrollView>
    </ViewVertical>
