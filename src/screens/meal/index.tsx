...
const MealScreen = ({route}: any) => {
  ...
  const mealRes = useSelector<RootState>(
    (state: RootState) => state.meal.data,
  ) as any;
  const isError = useSelector<RootState>(
    (state: RootState) => state.meal.status,
  ) as any;
    ...
    <ViewVertical style={styles.container}>
      <Header
        ...
        leftAction={() => {
          ...
          dispatch(getMealByDaySuccess('error'));
        }}
      />
      <ScrollView style={{flex: 1, padding: 20}}>
        ...
        <ViewVertical style={styles.blockDay}>
          <ViewHorizontal style={[styles.headerFlatCalendar]}>
            ...
            <Text style={[styles.textTitleFlat]}>{`${strings('WEEK')} ${moment()
              .add(indexWeek, 'week')
              .week()} (${moment()
              .add(indexWeek, 'week')
              .startOf('isoWeek')
            ...
            <TouchableOpacity
              ...
              <IconAnt
                ...
                size={20}
                color={
                  moment().week() === moment().add(indexWeek, 'week').week()
                    ? colors.gray
                    : colors.white
                }
              />
          ...
          <ViewHorizontal
            style={[
              styles.containerActivities,
              {borderWidth: 0, paddingLeft: 10},
            ]}>
            {listDayWeek?.map((item, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.itemDay,
                  currentDay.format('dd') == thingLanguage(item)
                ...
                <Text
                  style={[
                    ...
                    currentDay.format('dd') == thingLanguage(item)
                      ? {color: colors.white}
                      : {color: item.isDisable ? colors.gray : '#06313E'},
                  ]}>
                  {thingLanguage(item)}
                </Text>
              </TouchableOpacity>
            ...
            <ViewVertical style={styles.containerActivities}>
              {mealRes?.morning?.map((item: string, index: number) => (
                <ViewHorizontal style={styles.containerItem}>
                  ...
                  <ViewVertical>
                    <Text style={styles.txtVi}>
                      {splitString(item)[0] || ''}
                    </Text>
                    <Text style={styles.txtEn}>
                      {splitString(item)[1] || ''}
                    </Text>
                  </ViewVertical>
                </ViewHorizontal>
        ...
        {mealRes?.Notes ? (
          <ViewVertical style={styles.blockNotes}>
            ...
            <ViewVertical style={styles.containerActivities}>
              <Text style={styles.txtNote}>{mealRes.Notes || ''}</Text>
            </ViewVertical>
          </ViewVertical>
        ) : null}
      </ScrollView>
